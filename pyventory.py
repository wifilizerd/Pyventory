...
def Help(_menu):                                                #The Help Screen- broken down so specified potion can be called were needed.
    if _menu == 'main':                                             #Pyventory Main Menu Help
        ...
        print(
            ...
            "export     run Check and Export using the same inventory file used in Scan and Check.\n"
        )   
...
def Convert2List(_filename, _column, _savefile):                           #Read data from file and add it to a list.
    ...
    for row in list(_filereader):
        if row:                                                     #check if there is data on tha row.
            if row[int(_column)] == '':                                  #check if there is data in the field
                writefile(_savefile, 'csv', row)                 #write out any rows that do not have info in the selected column
            else:
...
def SCANandCHECK():                                             #Interface for scanning or entering information to check with,
    ...
    print('Noteing Recorde is disabled, to enable type "add" then enter')
    
    _assetLIST = []
    for row in Convert2List(_INVfile, _INVcol, _Savefile):                     # convert list of lists into a single list of specified column info.
    ...
    while True:                                                     #Start of teh entery process this is the loop that will take info and check in the inventory file.
        ...
        if _scan.lower() == 'add':                                  #add command used to turn on adding info if entery is not found.
            ...
            for row in Convert2List(_INVfile, _INVcol, _Savefile):
...
def CheckandExport():
    ...
    print('')
    print('press x then enter to exit')
    _INVlist = Convert2List(_INVfile, _INVcol, _scanfile[:-12])
    _scanlist = Convert2List(_scanfile, _scancol, _scanfile[:-12])
