...
def setupFiles():
    ...
    InventoryFile = window.filename = tkFileDialog.askopenfilename(initialdir = "/",title = "Select Inventory file",filetypes = (("csv files","*.csv"),("all files","*.*")))
    ...
    ScannedFile = window.filename = tkFileDialog.askopenfilename(initialdir = "/",title = "Select Scanned file",filetypes = (("csv files","*.csv"),("all files","*.*")))
...
def Interface():
    ...
    Button (window, text='Scan', justify='center', padx='2', command=SCANandCHECK).grid(row=1, column=0, stick=W)
